...
select hiredate, substr(hiredate, 1, 2) as year, substr(hiredate, 4, 2) as month from emp;
...
select ename, to_char(hiredate, 'YY') as "입사 년도", to_char(hiredate, 'MM') as "입사 월", to_char(hiredate, 'DY') as "입사 요일" from emp;
...
-- mysql 에서는 if 함수 -> if(조건, ture, false)
...
-- null 은 카운트대상이 되지 않으므로 주의 

select job, count(job) -- null 이 없으면?
from emp
-- where~~~~
group by job
-- having ~~~~
...
select deptno,
decode(deptno, 10, '10번 부서', 20, '20번 부서', 30, '30번 부서') as dname,
decode(deptno, 10, '서울', 20, '강동', 30, '천호') as rel,
count(ename), round(avg(sal)) as avgsal
from emp 
group by deptno
order by deptno
;
