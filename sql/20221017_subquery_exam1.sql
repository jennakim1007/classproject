...
from emp e, (select deptno, ename from emp where ename like '%K%') d
where e.deptno=d.deptno
;

-- 54. 부서위치가 DALLAS인 사원의 이름과 부서번호 및 담당업무를 표시하시오.
select e.ename, e.deptno, e.job
from emp e, (select deptno, loc from dept where loc='DALLAS') d
where e.deptno=d.deptno
;
...
from emp e, (select deptno, dname from dept where dname='RESEARCH') d
where e.deptno=d.deptno
;

-- 57. 평균 월급보다 많은 급여를 받고 이름에 M이 포함된 사원과 같은 부서에서 근무하는 사원의 사원 번호, 이름, 급여를 표시하시오.
select distinct e.empno, e.ename, e.sal
from emp e, (select deptno, ename from emp where ename like'%M%') e1
...
-- 49. 부하직원이 없는 사원의 이름을 표시하시오.
-- 나의 사원번호가 mgr 에 있다면 -> 나는 관리자 
-- 나의 사원번호가 mgr 에 없다면 -> 나는 부하직원 없음 
select *
from emp
